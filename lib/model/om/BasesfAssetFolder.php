...
abstract class BasesfAssetFolder extends BaseObject  implements Persistent
{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($isInsert) {
				...
				if (!$this->isColumnModified(sfAssetFolderPeer::CREATED_AT)) {
					$this->setCreatedAt(time());
				}
				if (!$this->isColumnModified(sfAssetFolderPeer::UPDATED_AT)) {
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(sfAssetFolderPeer::ID) ) {
	...
	{
		if (isset($alreadyDumpedObjects['sfAssetFolder'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['sfAssetFolder'][$this->getPrimaryKey()] = true;
		...
		if ($includeForeignObjects) {
			if (null !== $this->collsfAssets) {
				$result['sfAssets'] = $this->collsfAssets->toArray(null, true, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
			}
		}
		return $result;
	}
	...
	/**
	 * Clears out the collsfAssets collection
	 *
	...
	/**
	 * Initializes the collsfAssets collection.
	 *
	 ...
	 * @return     void
	 */
	public function initsfAssets($overrideExisting = true)
	{
		if (null !== $this->collsfAssets && !$overrideExisting) {
			return;
		}
		$this->collsfAssets = new PropelObjectCollection();
		$this->collsfAssets->setModel('sfAsset');
	}
	 ...
	 * @return     PropelCollection|array sfAsset[] List of sfAsset objects
	 * @throws     PropelException
	 */
	public function getsfAssets($criteria = null, PropelPDO $con = null)
	{
		if(null === $this->collsfAssets || null !== $criteria) {
			if ($this->isNew() && null === $this->collsfAssets) {
				// return empty collection
				$this->initsfAssets();
			} else {
				$collsfAssets = sfAssetQuery::create(null, $criteria)
					->filterBysfAssetFolder($this)
					->find($con);
				if (null !== $criteria) {
					return $collsfAssets;
				}
				$this->collsfAssets = $collsfAssets;
			}
		}
	...
	public function countsfAssets(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if(null === $this->collsfAssets || null !== $criteria) {
			if ($this->isNew() && null === $this->collsfAssets) {
				...
				$query = sfAssetQuery::create(null, $criteria);
				if($distinct) {
					$query->distinct();
				}
				return $query
			...
			return count($this->collsfAssets);
		}
	}
	...
	public function addsfAsset(sfAsset $l)
	{
		...
		if (!$this->collsfAssets->contains($l)) { // only add it if the **same** object is not already associated
			$this->collsfAssets[]= $l;
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			if ($this->collsfAssets) {
				foreach ($this->collsfAssets as $o) {
					$o->clearAllReferences($deep);
				}
			}
		} // if ($deep)

		if ($this->collsfAssets instanceof PropelCollection) {
			$this->collsfAssets->clearIterator();
		}
		$this->collsfAssets = null;
	}
	...
	{
		return (string) $this->exportTo(sfAssetFolderPeer::DEFAULT_STRING_FORMAT);
	}
	...
	{
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BasesfAssetFolder:' . $name))
		{
